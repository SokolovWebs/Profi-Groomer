...
// Получение всех элементов .picture
const pictureElements = document.querySelectorAll('.picture');

// Добавление обработчика события для каждого изображения
pictureElements.forEach(picture => {
    picture.addEventListener('click', function() {
        const img = this.querySelector('img');
        specialModalImage.src = img.src; // Устанавливаем источник изображения в новом модальном окне
        specialModal.style.display = "block"; // Показываем новое модальное окно
    });
});

// Закрытие нового модального окна
closeSpecialModal.addEventListener('click', function() {
    specialModal.style.display = "none"; // Скрываем новое модальное окно
});

// Закрытие нового модального окна при клике вне изображения
specialModal.addEventListener('click', function() {
    specialModal.style.display = "none"; // Скрываем новое модальное окно
});
